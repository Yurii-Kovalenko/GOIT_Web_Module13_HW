...
class Auth:
    ...
    async def create_access_token(self, data_payload: dict, expires_delta_sec: Optional[float] = None):
    ...
    async def create_refresh_token(self, data_payload: dict, expires_delta_sec: Optional[float] = None):
        to_encode = data_payload.copy()
        if expires_delta_sec:
            expire = datetime.now() + timedelta(seconds=expires_delta_sec)
        else:
